...
`contracts/action-tool-lock.json` owns action SHAs, human-readable releases, runtime generations, approved internal-action roots, the exact parser version, source digest, and runtime-specific wheel URL and digest. `contracts/validation-harness.json` owns thresholds, semantic runner bootstrap profiles, required fixture coverage, and narrowly documented temporary exceptions. Runner contract files containing `runner` in their filename are discovered automatically so issue-specific capability work extends the gate without changing `self-check.yml`.
