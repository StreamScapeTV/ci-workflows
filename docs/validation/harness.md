...
`contracts/action-tool-lock.json` owns action SHAs, human-readable releases, runtime generations, approved internal-action roots, the exact parser version, source digest, and runtime-specific artifact URLs and digests. `contracts/validation-harness.json` owns thresholds, semantic runner bootstrap profiles, required fixture coverage, and narrowly documented temporary exceptions.
