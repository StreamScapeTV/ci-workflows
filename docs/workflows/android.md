...
For `protected-full`, dependency resolution is deliberately moved to the front of the pipeline. `actions/warm-gradle-dependencies` runs against a disposable exact-source copy and records only bounded cache mode/wall-time evidence. It passes the same filtered `GRADLE_USER_HOME`, fixed read-only seed, Android SDK/JDK state, and verified `CI_PRIVATE_DEPENDENCY_PATH` that the subsequent Android execution uses, while filtering GitHub/private-dependency credentials. The warm source copy is removed before the build continues, so Gradle's generated verification metadata cannot modify the admitted checkout.
...
When a private dependency is requested, it is checked out once at the exact planned revision before dependency warming and only its verified path is passed into the warm/build runtimes. The dependency token is confined to that checkout step and is not forwarded to Gradle or either cache sync.
