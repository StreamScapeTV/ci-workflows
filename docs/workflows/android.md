...
## Optional trusted seed warming

`promote_gradle_seed` is an acceleration-only switch. It is `false` by default and never changes validation scope or task selection. When a caller explicitly sets it to `true`, the reusable workflow waits for the Android execute phase plus copied-source cleanup and Android residue verification to succeed, then invokes the immutable issue #347 client against the **same marker-bound private `GRADLE_USER_HOME` that just executed validation**. No second Gradle home, runner job, artifact handoff, GitHub cache, caller-selected endpoint, PAT, deploy key, S3, or OCI transport is introduced.

The reusable workflow declares `id-token: write` only because the optional client may request GitHub OIDC. Reusable workflows cannot elevate token permissions beyond their caller. A normal caller that grants only `contents: read` therefore retains `id-token: none`; a trusted caller must separately grant `id-token: write` to the exact reusable-workflow call that enables `promote_gradle_seed`. The seed client itself fixes audience, endpoint, candidate path policy, framing, file/hash bounds, and OIDC request semantics; the caller supplies only the admitted source SHA.

Promotion uses `continue-on-error` so cache availability never becomes product correctness. The seed action nevertheless performs its own terminal marker-bound workspace cleanup under `if: always()`. If that cleanup cannot prove completion, the reusable workflow invokes the ordinary immutable workspace cleanup as a fallback. The final Android result deliberately ignores upload success/failure, but it still requires one verified cleanup path, exact-source cleanliness, Android cleanup/residue, and the authoritative validation result. This keeps a promoter/network/OIDC failure non-fatal while refusing to hide unverified local residue.
