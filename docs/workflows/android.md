...
Only a fixed non-secret runtime environment is forwarded to product execution: isolated `HOME`, `TMPDIR`, `GRADLE_USER_HOME`, runner `PATH`, JDK/Android SDK locations, fixed `C.UTF-8` locale, UTC, and an optional verified private-dependency path. GitHub tokens, private dependency checkout tokens, arbitrary caller environment, workflow metadata, and secret-bearing variables are not forwarded. Gradle always receives `--no-daemon`.
