...
Private same-organization callers do not provide a credential for `StreamScapeTV/ci-workflows`, and the reusable workflow does not clone the private central repository with the caller-scoped Actions token. Instead it invokes the reviewed central composite actions directly at immutable revision `70e08d4ddf8930046632a7135950e924b82e22bf`. The caller cannot select that revision, and no `central_source`, generic checkout token, PAT, mutable helper ref, or `secrets: inherit` surface exists.
