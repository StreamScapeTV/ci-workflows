...
After the warm phase succeeds, Central immediately attempts one best-effort `caches/modules-*` delta sync while the job-private `GRADLE_USER_HOME` is still available. The normal Android validation then proceeds in the same executor/private Gradle home. Dependency-resolution failure blocks the requested work early; cache-promotion failure does not, because the updater remains acceleration-only and `continue-on-error`.
...
Routine Android validation requires no GitHub OIDC permission. `protected-full` warms automatically. A `gradle` maintenance call with the exact private dependency explicitly invokes the same warm primitive so cache maintenance can be run without Media publication or application compile/test/lint/assemble work. Ordinary `compile`, unit, lint, assemble, targeted-unit, and script scopes do not add this extra dependency-resolution pass.
...
The dependency warm phase reports `gradle_dependency_cache_mode=read-only-seed|cold` and bounded warm wall time before the requested Android work begins. Terminal logging projects only those two fields. This makes the dependency bootstrap cost separately observable from Media preparation and application compilation.
