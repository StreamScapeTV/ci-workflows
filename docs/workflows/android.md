...
Routine Android validation requires no GitHub OIDC permission. PR, manual, work-branch, and integration runs use the same cache read/update model. Dependency warming runs automatically for `protected-full` and `compile`; narrower unit/lint/assemble/script scopes avoid that extra dependency-resolution pass.
