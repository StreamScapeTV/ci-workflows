...
Shared Gradle seed warming is deliberately **not** a public workflow input. The reusable Android job requests `id-token: write`, but GitHub's reusable-workflow permission ceiling prevents a called workflow from elevating above its caller. Normal PR, manual, and work-branch callers grant only `contents: read`, so their effective OIDC authority remains absent.
