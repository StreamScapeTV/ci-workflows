...
- The external service deployed by Flux for issue #495 is the standalone transport-only project under `ci-broker/`. Its complete deployed application runtime is `ci-broker/app.py`, its image definition is `ci-broker/Containerfile`, and its Helm chart is `ci-broker/chart/`. The broker project must not import or package `src/ci_workflows/`. It may authenticate the Supabase INSERT webhook, claim one bounded CI request, validate the reviewed workflow/profile, reject/cancel an invalid or definitively failed dispatch, and fire-and-forget the fixed Central workflow. Its public GitHub dispatch payload contains only the opaque `ci_run_id` and opaque hashed active identity. It must not resolve or check out source, read product configuration, admit dependencies, execute product logic, discover GitHub runs, process build logs, persist logs, access R2, or own normal lifecycle after dispatch.
...
- Central re-claims the canonical request internally by opaque CI UUID, records the actual GitHub run identity itself, resolves the human branch/tag inside the trusted executor, records the observed SHA as evidence only, and executes the existing canonical technology validation functions. Detailed private checkout/build/test/cleanup stdout and stderr stay runner-local; they are gzip-compressed, uploaded to private Cloudflare R2, downloaded again, and SHA-256 verified before terminal Agent State status is written. Agent State stores no log body, only bounded status/error plus the R2 receipt/status compatibility fields.
...
- Python functions under `src/ci_workflows/` are the Central implementation layer. Workflow YAML owns orchestration only; composite actions and CLI adapters stay thin and delegate reusable behavior to named tested functions. The standalone `ci-broker/app.py` is intentionally not a Central implementation module and owns only its bounded transport behavior.
...
### Inventory-first, no-redundancy implementation rule

- Root `PYTHON_INVENTORY.yml` is the mandatory implementation inventory for `src/ci_workflows/`. Read it before creating or substantially modifying Python implementation. The inventory is an architecture/navigation authority for what already exists; the issue and repository rules remain the authority for intended behavior.
- **Reuse or refactor before creating.** If an existing module, class, function, provider, adapter, contract, execution path, helper, or primitive already owns the requested responsibility, extend/refactor that implementation. Do not create a parallel implementation merely because the current issue, product, workflow, or caller has a different name.
- **No duplicated cross-cutting infrastructure.** R2 storage/authentication/upload/download/read-back/digest verification, Agent State CI lifecycle/RPC access, GitHub source access, workspace handling, process execution, credentials, cleanup, evidence, and release mechanics belong to product-neutral domain providers. Technology modules such as Apple, Android, Python, or Node must consume those providers rather than reimplementing or privately wrapping the same infrastructure per family.
- **Prefer cohesive domain modules over file proliferation.** The target structure is recognizable by responsibility: e.g. `r2.py` for R2 capabilities, `agent_state.py` for Agent State CI capabilities, `source.py` for source handling, `workspace.py` for workspace handling, `release.py` for common release mechanics, and technology modules such as `apple.py`, `android.py`, `python.py`, and `node.py` for technology-specific validation/build functions. One module may and should contain multiple related functions/classes when that is clearer than splitting every policy/type/execution/helper into separate files.
- A domain/build/release concern should normally require **one implementation module** and may use a second implementation module only when there is a clear runtime or responsibility boundary. If a proposed domain, build, validation, publication, or release process genuinely requires **more than two Python implementation modules**, stop before adding the third, surface it to the owner, and document in the GitHub issue why one or two cohesive modules are insufficient. Do not silently grow `*_contract.py`, `*_execution.py`, `*_types.py`, `*_primitives.py`, `ciw_*.py`, or similar parallel layers.
- Do not treat smaller files as an architectural goal. Splitting a cohesive responsibility across files without a real boundary is fragmentation, not separation of concerns. Conversely, do not create a God module: when a real second boundary exists, document it and keep dependency direction from generic infrastructure toward thin technology adapters, never from generic infrastructure into a product/technology-specific implementation merely to reuse private helpers.
- Before implementing a new function/class, search the inventory and current source for equivalent or overlapping behavior. If overlap exists, consolidate at the owning domain and migrate callers rather than copying behavior.
- Every finalized change that adds, removes, renames, moves, or materially changes a Python module/class/function must update the matching entry in `PYTHON_INVENTORY.yml` in the same change. Inventory drift is a validation defect.
- Until the initial complete inventory has been generated and reviewed, **do not add a new `src/ci_workflows/*.py` module without explicit owner approval**. Add new behavior to the closest existing owner and record it in the inventory.
- Refactors must reduce or preserve implementation surface; do not add a new abstraction layer while leaving the superseded layer permanently active. Compatibility shims must be explicitly marked in the inventory with a removal/migration target.
