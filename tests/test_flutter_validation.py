...
class FakeRunner(CommandRunner):
    def __init__(self, runtime: Mapping[str, object], *, fail_at: str = "", mutate_lock: bool = False) -> None:
    ...
    def run(self, argv: Sequence[str], *, cwd: Path, env: Mapping[str, str]) -> CommandOutcome:
        ...
        if command[:3] == ("flutter", "build", "ios") and "--no-codesign" in command:
...
def request(profile: str, consumer: str = "directus-canonical") -> FlutterRequest:
    return FlutterRequest(SHA, consumer, FlutterProfile(profile), "trusted-pr")
...
class FlutterValidationTests(unittest.TestCase):
    ...
    def test_contract_is_deterministic_and_immutable_setup_is_pinned(self) -> None:
        first = json.dumps(self.contract, sort_keys=True, separators=(",", ":"))
        second = json.dumps(flutter.load_flutter_contract(ROOT), sort_keys=True, separators=(",", ":"))
    ...
    def test_pin_malformed_range_alias_channel_and_multiple_values_fail(self) -> None:
        ...
        for value in ("not-json", '{"flutter":"^3.41.4"}', '{"flutter":"stable"}', '{"flutter":"3.41.4","channel":"stable"}', '{"flutter":["3.41.4","3.44.6"]}'):
            with self.subTest(value=value):
                source = self.fixture_copy("finance")
                (source / ".fvmrc").write_text(value, encoding="utf-8")
                self.assert_code("invalid_runtime_source", lambda: discover_flutter_pin(source, consumer))
                shutil.rmtree(source)

    def test_pin_agreement_and_mismatch(self) -> None:
        source = self.fixture_copy("directus")
        (source / ".fvmrc").write_text('{"flutter":"3.44.6"}\n', encoding="utf-8")
        pin = discover_flutter_pin(source, self.contract["consumer_contracts"]["directus-canonical"])
        self.assertEqual((".flutter-version", ".fvmrc"), pin.sources)
        (source / ".fvmrc").write_text('{"flutter":"3.41.4"}\n', encoding="utf-8")
        self.assert_code("runtime_pin_mismatch", lambda: discover_flutter_pin(source, self.contract["consumer_contracts"]["directus-canonical"]))
    ...
    def test_symlinked_pin_and_escaped_gate_fail(self) -> None:
        ...
        (self.temp / "outside-version").write_text("3.44.6\n", encoding="utf-8")
        self.assert_code("path_rejected", lambda: discover_flutter_pin(source, self.contract["consumer_contracts"]["directus-canonical"]))
        modified = copy.deepcopy(self.contract)
        modified["consumer_contracts"]["directus-canonical"]["profiles"]["canonical-gate"]["gate_path"] = "../outside.sh"
        self.assert_code("path_rejected", lambda: build_plan(modified, request("canonical-gate"), source))

    def test_runtime_and_dart_identity_are_exact(self) -> None:
        toolchain = build_plan(self.contract, request("quality"), None).toolchain
        identity = parse_runtime_identity(json.dumps(runtime("runtime-3.44.6.json")), toolchain)
        self.assertEqual("3.12.2", identity["dart_version"])
        bad = runtime("runtime-3.44.6.json")
        bad["dartSdkVersion"] = "3.12.1"
        self.assert_code("dart_mismatch", lambda: parse_runtime_identity(json.dumps(bad), toolchain))
        bad = runtime("runtime-3.44.6.json")
        bad["frameworkVersion"] = "3.44.5"
        self.assert_code("runtime_mismatch", lambda: parse_runtime_identity(json.dumps(bad), toolchain))

    def test_runner_separation_and_source_audit_no_install(self) -> None:
        cases = {"source-audit": RunnerCapability.PORTABLE, "device-handoff": RunnerCapability.PORTABLE, "quality": RunnerCapability.MOBILE, "canonical-gate": RunnerCapability.MOBILE, "android-debug": RunnerCapability.MOBILE, "compatibility-smoke": RunnerCapability.MOBILE, "ios-simulator": RunnerCapability.APPLE}
        ...
        self.assertNotEqual(build_plan(self.contract, request("android-debug"), None).runner_profile, build_plan(self.contract, request("ios-simulator"), None).runner_profile)

    def test_quality_stage_order_and_optional_node_composition(self) -> None:
        plan = build_plan(self.contract, request("quality"), None)
        self.assertEqual(("runtime-verify", "dependency-restore", "quality", "tests", "cleanup"), tuple(stage.value for stage in plan.stages))
        finance = build_plan(self.contract, request("quality", "finance-embedded-web"), None)
        self.assertEqual("source-audit", finance.node_composition["command_profile"])
    ...
    def test_checked_in_gate_and_device_handoff_are_bounded(self) -> None:
        source = self.fixture_copy("directus")
        gate = build_plan(self.contract, request("canonical-gate"), source)
        self.assertEqual("tool/ci_gate.sh", gate.gate_path)
        handoff = build_plan(self.contract, request("device-handoff"), source)
    ...
    def test_lockfile_mutation_command_failure_dirty_source_and_cleanup_failure(self) -> None:
        source = self.fixture_copy("directus")
        self.assert_code("lockfile_drift", lambda: flutter.validate(contract_root=ROOT, source_root=source, state_root=self.temp / "state-lock", request=request("quality"), phase="execute", runner=FakeRunner(runtime("runtime-3.44.6.json"), mutate_lock=True)))
        source = self.fixture_copy("directus")
        self.assert_code("command_failed", lambda: flutter.validate(contract_root=ROOT, source_root=source, state_root=self.temp / "state-command", request=request("quality"), phase="execute", runner=FakeRunner(runtime("runtime-3.44.6.json"), fail_at="analyze")))
        source = self.fixture_copy("directus")
        ...
        self.assert_code("dirty_source", lambda: flutter.validate(contract_root=ROOT, source_root=source, state_root=self.temp / "state-dirty", request=request("source-audit"), phase="execute", runner=FakeRunner(runtime("runtime-3.44.6.json"))))
    ...
    def test_caller_selected_runner_device_engine_registry_and_deployment_fail(self) -> None:
        ...
        for key in ("RUNNER", "DEVICE", "ENGINE", "REGISTRY", "DEPLOYMENT", "DOWNLOAD_URL", "RUNTIME"):
            with self.subTest(key=key):
                ...
                self.assert_code("forbidden_input", lambda environment=environment: flutter.request_from_environment(environment, self.contract))
