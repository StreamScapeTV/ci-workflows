...
BOOTSTRAP_BLOB_SHA1 = "1770311f5b3998b5dbf3f8ee191acd419aa52a56"
GUIDED_RUN_FLOW_PATH = (
    "apple/Tests/StreamscapePlaybackLabSupportTests/"
    "PlaybackLabGuidedAcceptanceRunFlowTests.swift"
)
GUIDED_RUN_FLOW_BLOB_SHA1 = "2a9149b864bf59079099035c15af54234f54b452"
MEDIA_PATHS = {
    BOOTSTRAP_PATH: BOOTSTRAP_BLOB_SHA1,
    GUIDED_RUN_FLOW_PATH: GUIDED_RUN_FLOW_BLOB_SHA1,
}
MEDIA_TASKS = {
...
class AndroidMediaSourcePolicyTests(unittest.TestCase):
    ...
    def test_current_media_sentinel_blobs_are_exactly_profile_bounded(self) -> None:
        ...
        for profile in MEDIA_TASKS:
            with self.subTest(profile=profile):
                ...
                self.assertEqual(active, MEDIA_PATHS)
        ...
        for profile in sorted(android_policy._ANDROID_PROFILES - set(MEDIA_TASKS)):
            with self.subTest(disallowed_profile=profile):
                ...
                self.assertTrue(MEDIA_PATHS.keys().isdisjoint(active))

    def test_changed_media_sentinel_blob_with_real_token_shape_fails_closed(self) -> None:
        for media_path in MEDIA_PATHS:
            with self.subTest(path=media_path), tempfile.TemporaryDirectory() as temporary:
