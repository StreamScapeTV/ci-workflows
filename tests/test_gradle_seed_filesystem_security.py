...
class GradleSeedFilesystemSecurityTests(unittest.TestCase):
    ...
    def test_directory_symlink_swap_between_stat_and_open_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            def swap_then_open(parent_fd, name, *, expected=None, code="gradle_seed_path_rejected"):
                nonlocal swapped
    ...
    def test_each_absolute_gradle_home_component_is_opened_without_following_symlinks(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_response_counts_require_json_integers_not_float_equivalents(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            home = Path(directory) / "gradle"
            write_seed(home, "modules-2/a.bin", b"seed")
            files = gradle_seed.collect_seed_files(home)
