...
class RelayRequestTests(unittest.TestCase):
    ...
    def test_reviewed_starter_semantic_families_share_one_host_profile(self) -> None:
        for workflow_key in ("validation.apple", "validation.android", "validation.python"):
            with self.subTest(workflow_key=workflow_key):
                request = RelayRequest.from_claimed_run(claimed_run(workflow_key=workflow_key))
                self.assertEqual(request.workflow_key, workflow_key)
                self.assertEqual(request.profile, "host")
                self.assertEqual(set(request.workflow_inputs()), {"active_key", "ci_run_id"})

    def test_unreviewed_workflow_profile_or_inputs_fail_closed(self) -> None:
        cases = (
            ({"workflow_key": "validation.native"}, "unsupported_ci_intent"),
            ({"workflow_key": "validation.script"}, "unsupported_ci_intent"),
            ({"test_profile": "release"}, "unsupported_ci_intent"),
            ({"workflow_key": "validation.android", "test_profile": "mobile"}, "unsupported_ci_intent"),
            ({"inputs": {"scope": "integration"}}, "unsupported_ci_inputs"),
        )
        for patch, code in cases:
            with self.subTest(patch=patch), self.assertRaisesRegex(BrokerError, code):
    ...
    def test_tag_is_explicit_and_not_guessed_from_ref_shape(self) -> None:
        request = RelayRequest.from_claimed_run(claimed_run(ref="release/fancy-tag", is_tag=True))
...
class ThinCiRelayTests(unittest.TestCase):
    ...
    def test_webhook_claims_once_and_dispatches_only_opaque_inputs(self) -> None:
        ...
        state.claim_result = {"ok": True, "run": claimed_run(workflow_key="validation.python"), "replayed": False}
        ...
        self.assertNotIn("validation.python", rendered)
    ...
    def test_replayed_nonaccepted_request_never_dispatches_duplicate_pipeline(self) -> None:
        relay, state, dispatch = self.make_relay()
        state.claim_result = {"ok": True, "run": claimed_run(status="running"), "replayed": True}
        result = relay.handle_agent_state_webhook(
            webhook(),
            {"X-StreamScape-Webhook-Secret": "agent-state-webhook-secret"},
        )
    ...
    def test_invalid_claimed_intent_is_cancelled_without_dispatch(self) -> None:
        ...
        state.claim_result = {
            "ok": True,
            "run": claimed_run(workflow_key="validation.native"),
            "replayed": False,
        }
        ...
        self.assertEqual(
            state.transitions,
            [("00000000-0000-4000-8000-000000000001", {"status": "cancelled", "error_summary": "unsupported_ci_intent"})],
        )
