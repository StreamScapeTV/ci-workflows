...
class GithubOidcRequesterTests(unittest.TestCase):
    def test_requests_only_exact_audience_without_caller_endpoint_authority(self) -> None:
        response = mock.Mock()
        response.status = 200
    ...
    def test_flux_uploader_uses_only_fixed_internal_service(self) -> None:
        response = mock.Mock()
        response.status = 200
...
class GradleSeedPromotionTests(unittest.TestCase):
    ...
    def test_protected_push_upload_verifies_response_and_emits_redacted_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            environment.update(
                {
                    "GITHUB_REPOSITORY": "StreamScapeTV/example-consumer",
                    "GITHUB_REF": "refs/heads/integration",
                    "GITHUB_WORKFLOW_REF": (
                        "StreamScapeTV/example-consumer/.github/workflows/ci.yml"
    ...
    def test_context_is_product_neutral_but_rejects_pr_dispatch_and_sha_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            home = self._seed_home(directory)
            ...
            accepted.update(
                {
                    "GITHUB_REPOSITORY": "StreamScapeTV/another-product",
                    "GITHUB_REF": "refs/heads/whatever",
                    "GITHUB_WORKFLOW_REF": "any/reviewed/workflow@refs/heads/whatever",
                }
            )
            ...
            cases = (
                {**base, "GITHUB_EVENT_NAME": "pull_request"},
                {**base, "GITHUB_EVENT_NAME": "workflow_dispatch"},
                {**base, "GITHUB_REF_TYPE": "tag"},
                {**base, "GITHUB_SHA": "b" * 40},
            )
