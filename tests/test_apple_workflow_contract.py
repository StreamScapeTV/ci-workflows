...
class AppleWorkflowContractTests(unittest.TestCase):
    def setUp(self) -> None:
        self.contract = json.loads((ROOT / "contracts/apple-validation.json").read_text(encoding="utf-8"))
        self.workflow = (ROOT / ".github/workflows/reusable-apple.yml").read_text(encoding="utf-8")
        self.smoke = (ROOT / ".github/workflows/apple-validation-smoke.yml").read_text(encoding="utf-8")
        self.action = (ROOT / "actions/validate-apple/action.yml").read_text(encoding="utf-8")
        self.checkout_cleanup_adapter = (ROOT / "scripts/ci/apple_checkout_cleanup.py").read_text(encoding="utf-8")
        self.facade = (ROOT / "src/ci_workflows/apple.py").read_text(encoding="utf-8")
        self.planner = (ROOT / "src/ci_workflows/apple_contract.py").read_text(encoding="utf-8")
        self.execution = (ROOT / "src/ci_workflows/apple_execution.py").read_text(encoding="utf-8")
        self.types = (ROOT / "src/ci_workflows/apple_types.py").read_text(encoding="utf-8")
    ...
    def test_public_api_and_stable_check(self) -> None:
        ...
        expected_inputs = {"admitted_sha", "artifact_exception_id", "command_profile", "destination_profile", "platform", "scheme", "script_path", "validation_profile", "version_file", "working_directory"}
        block = self.workflow.split("inputs:", 1)[1].split("outputs:", 1)[0]
        self.assertEqual(expected_inputs, set(re.findall(r"^      ([a-z_]+):$", block, re.M)))
        output_block = self.workflow.split("outputs:", 1)[1].split("permissions:", 1)[0]
        self.assertEqual({"artifact_exception_used", "result", "test_summary"}, set(re.findall(r"^      ([a-z_]+):$", output_block, re.M)))
    ...
    def test_semantic_runner_selection_uses_protected_planner(self) -> None:
        ...
        self.assertEqual(1, self.workflow.count("runs-on: ${{ fromJSON(needs.plan.outputs.runs_on_json) }}"))
        self.assertEqual(3, self.smoke.count("runs-on: ${{ fromJSON(needs.plan.outputs.runs_on_json) }}"))
        ...
        self.assertIn('requested_profile=plan.runner_profile.value', (ROOT / "src/ci_workflows/ciw_apple.py").read_text(encoding="utf-8"))
    ...
    def test_smoke_exercises_exact_implementation_on_all_apple_platforms(self) -> None:
        ...
        self.assertGreaterEqual(self.smoke.count("uses: ./.ciw/actions/validate-apple"), 12)
    ...
    def test_smoke_cancellation_scope_is_stable_and_skips_artifact_check(self) -> None:
        ...
        self.assertIn("group: apple-validation-smoke-pr-${{ github.event.pull_request.number }}", concurrency)
    ...
    def test_smoke_runs_when_apple_public_registration_changes(self) -> None:
        for path in ("contracts/bootstrap-public-workflows.json", "contracts/ciw-commands.json", "contracts/public-workflows.json", "contracts/public-workflows/validation.json", "docs/reference/ciw.md", "docs/workflows/public-api-reference.md", "src/ci_workflows/ciw.py", "tests/test_apple_ciw_dispatch.py", "tests/test_bootstrap.py", "tests/test_ciw_cli.py", "tests/test_ciw_contracts.py", "tests/test_public_api_contract.py"):
            with self.subTest(path=path):
                self.assertIn(f"      - {path}", self.smoke)

    def test_external_actions_are_full_sha_pinned(self) -> None:
        for value in re.findall(r"uses:\s*([^\s#]+)", "\n".join((self.workflow, self.smoke))):
    ...
    def test_private_helper_identity_and_terminal_cleanup_are_mandatory(self) -> None:
        ...
        self.assertNotIn("python3 .ciw/scripts/ci/apple_checkout_cleanup.py", self.workflow)
        self.assertEqual(4, self.workflow.count(f"uses: StreamScapeTV/ci-workflows/actions/validate-apple@{APPLE_HELPER_SHA}"))
        self.assertIn(f"uses: StreamScapeTV/ci-workflows/actions/exact-checkout@{FOUNDATION_SHA}", self.workflow)
        self.assertIn(f"uses: StreamScapeTV/ci-workflows/actions/prepare-workspace@{FOUNDATION_SHA}", self.workflow)
        self.assertIn(f"uses: StreamScapeTV/ci-workflows/actions/cleanup-workspace@{FOUNDATION_SHA}", self.workflow)
    ...
    def test_media_vlc_tvos_native_contract_is_bounded_and_distinct_from_mpv(self) -> None:
        ...
        self.assertEqual(consumers["streamscape-media-apple"]["profiles"]["native-dependency-preparation"], "media-native-dependency")
        self.assertEqual(consumers["streamscape-media-vlc-tvos-apple"]["repository"], "StreamScapeTV/streamscape-media")
        self.assertEqual(consumers["streamscape-media-vlc-tvos-apple"]["profiles"], {"native-dependency-preparation": "media-vlc-tvos-native-dependency"})
        request = AppleValidationRequest(repository="StreamScapeTV/streamscape-media", admitted_sha="a" * 40, consumer_contract="streamscape-media-vlc-tvos-apple", validation_profile=AppleProfile.NATIVE_DEPENDENCY_PREPARATION, source_trust="trusted-exact", platform="apple-native")
        ...
        self.assertEqual(plan.commands[0].script_path, "scripts/ci/build-private-tvos-vlc-candidate.sh")
        ...
        self.assertIn("native/vlc/apple-tvos-build-support-inventory.json", plan.protected_paths)
        self.assertEqual(plan.environment_bindings, (("STREAMSCAPE_ARTIFACT_DIR", "native-output"),))

    def test_checkout_cleanup_is_fixed_and_no_follow(self) -> None:
        workflows = self.workflow + self.smoke
        self.assertNotIn("python3 .ciw/scripts/ci/apple_checkout_cleanup.py", self.workflow)
        self.assertIn("python3 .ciw/scripts/ci/apple_checkout_cleanup.py source", self.smoke)
        ...
        self.assertIn('_TARGETS = {"central": ".ciw", "source": "source"}', self.checkout_cleanup_adapter)
    ...
    def test_no_signing_physical_device_archive_store_or_deployment_path(self) -> None:
        ...
        for forbidden in ("secrets.", "keychain import", "provisioning_profile", "development_team", "archivepath", "exportarchive", "testflight", "notarytool", "kubectl", "helm", "docker", "buildah", "device_udid"):
    ...
    def test_exact_toolchain_sdk_and_package_resolution_are_checked(self) -> None:
        ...
        self.assertEqual(set(toolchain["sdk_versions"]), {"iphoneos", "iphonesimulator", "appletvos", "appletvsimulator", "macosx"})
    ...
    def test_no_product_name_branching_in_shared_implementation(self) -> None:
        shared = "\n".join((self.facade, self.planner, self.execution, self.types, (ROOT / "src/ci_workflows/ciw_apple.py").read_text(encoding="utf-8"), (ROOT / "src/ci_workflows/apple_contract_fragments.py").read_text(encoding="utf-8"))).lower()
    ...
    def test_fixture_is_product_neutral_and_unsigned(self) -> None:
        project = (ROOT / "tests/fixtures/apple-validation/smoke-project/AppleValidationSmoke.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
        source = (ROOT / "tests/fixtures/apple-validation/smoke-project/Sources/SmokeApp.swift").read_text(encoding="utf-8")
