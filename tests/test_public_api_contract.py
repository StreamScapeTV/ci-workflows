...
from ci_workflows import public_api_contract as contract  # noqa: E402
...
class PublicApiContractTests(unittest.TestCase):
    ...
    def test_privileged_apis_require_immutable_references(self) -> None:
        for api, row in self.workflows.items():
            trust = row["trust_class"]
            if trust not in contract.PRIVILEGED_TRUST:
                continue
            case = {
                "api_name": api,
                "trust_class": trust,
    ...
    def test_self_check_runs_validator_renderer_and_tests(self) -> None:
        ...
        for required in (
            ...
            "python3 scripts/ci/public_api_contract.py render --check",
