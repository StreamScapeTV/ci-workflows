...
class ExactCheckoutTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        subprocess.run(["git", "init", "-q", str(self.origin_work)], check=True)
        subprocess.run(
            ["git", "-C", str(self.origin_work), "config", "user.email", "test@example.com"],
            check=True,
        )
        subprocess.run(
            ["git", "-C", str(self.origin_work), "config", "user.name", "Source Tests"],
            check=True,
        )
        (self.origin_work / "value.txt").write_text("one\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(self.origin_work), "add", "value.txt"], check=True)
        subprocess.run(["git", "-C", str(self.origin_work), "commit", "-qm", "one"], check=True)
        self.first = subprocess.check_output(
            ["git", "-C", str(self.origin_work), "rev-parse", "HEAD"], text=True
        ).strip()
        (self.origin_work / "value.txt").write_text("two\n", encoding="utf-8")
        subprocess.run(["git", "-C", str(self.origin_work), "commit", "-qam", "two"], check=True)
        self.second = subprocess.check_output(
            ["git", "-C", str(self.origin_work), "rev-parse", "HEAD"], text=True
        ).strip()
        subprocess.run(
            ["git", "clone", "-q", "--bare", str(self.origin_work), str(self.origin_bare)],
            check=True,
        )
    ...
    def checkout(self, sha: str, path: str = "source") -> dict[str, str]:
        return dict(
    ...
    def test_historical_exact_sha_is_detached_and_verified(self) -> None:
        ...
        persisted = subprocess.run(
            ["git", "-C", str(target), "config", "--local", "--get-regexp", r"http\..*extraheader"],
    ...
    def test_mutable_ref_malformed_sha_path_escape_and_changed_checkout_fail(self) -> None:
        for sha, expected in (
        ...
        occupied.mkdir()
        (occupied / "changed.txt").write_text("caller state\n", encoding="utf-8")
        with self.assertRaises(source.SourceAdmissionError) as caught:
            self.checkout(self.first, "occupied")
        self.assertEqual(caught.exception.instruction, "checkout_path_not_empty")

    def test_different_exact_commit_never_silently_reuses_prior_checkout(self) -> None:
