-- Issue #52: normalized private Supabase Agent State schema and helpers.
...
create table agent_private.projects (
  ...
  constraint projects_project_key_format
    check (project_key ~ '^[A-Za-z0-9][A-Za-z0-9-]{0,62}$'),
  constraint projects_repository_format
    check (repository_full_name ~ '^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$'),
  constraint projects_branch_format check (
    ...
    and pg_catalog.right(integration_branch, 1) not in ('/', '.')
  ),
  constraint projects_base_sha_format
...
create table agent_private.profiles (
  ...
  constraint profiles_key_values check (profile_key in ('agent', 'codex')),
  constraint profiles_display_values check (display_name in ('Agent', 'Codex')),
  constraint profiles_prefix_values check (id_prefix in ('gpt-agent', 'cod-agent'))
);
...
create table agent_private.project_slots (
  ...
  constraint project_slots_range check (slot between 1 and 99)
);
...
create table agent_private.work_sessions (
  ...
  constraint work_sessions_name_format
    check (session_name ~ '^(Agent|Codex) ([1-9][0-9]?)$'),
  constraint work_sessions_agent_id_format
    check (agent_id ~ '^(gpt-agent|cod-agent)-[1-9][0-9]?-[0-9]{8}-[0-9]{4}-[a-z0-9]{4}$'),
  constraint work_sessions_task_length check (pg_catalog.length(task) between 1 and 2000),
  constraint work_sessions_status_values
    check (status in ('active', 'blocked', 'review', 'done', 'cancelled')),
  constraint work_sessions_blocked_reason_length
    check (blocked_reason is null or pg_catalog.length(blocked_reason) between 1 and 2000),
  constraint work_sessions_terminal_summary_length
    check (terminal_summary is null or pg_catalog.length(terminal_summary) between 1 and 2000),
  constraint work_sessions_terminal_shape check (
...
create table agent_private.work_bindings (
  ...
  constraint work_bindings_issue_positive check (issue_number > 0),
  constraint work_bindings_pr_positive check (pr_number is null or pr_number > 0),
  constraint work_bindings_nonce_format check (branch_nonce ~ '^[a-z0-9]{4}$'),
  constraint work_bindings_branch_format check (
    branch ~ '^issue/[1-9][0-9]*-[a-z0-9][a-z0-9-]{0,180}-[a-z0-9]{4}$'
    and branch !~ '\.\.'
    and branch !~ '//'
    and branch !~ '@\{'
    and branch !~ '(^|/)[^/]*\.lock(/|$)'
    and pg_catalog.right(branch, 1) not in ('/', '.')
  )
);
...
create table agent_private.work_evidence (
  ...
  foreign key (session_id, project_id)
    references agent_private.work_sessions(id, project_id) on delete restrict,
  constraint work_evidence_base_sha_format check (base_sha ~ '^[0-9a-f]{40}$'),
  constraint work_evidence_head_sha_format
    check (head_sha is null or head_sha ~ '^[0-9a-f]{40}$'),
  constraint work_evidence_merge_sha_format
...
create table agent_private.requests (
  ...
  agent_id text,
  created_at timestamptz not null default pg_catalog.clock_timestamp(),
  constraint requests_id_format
    check (request_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$'),
  constraint requests_hash_format check (request_hash ~ '^[0-9a-f]{64}$'),
  constraint requests_action_values check (
...
create table agent_private.command_receipts (
  ...
  created_at timestamptz not null default pg_catalog.clock_timestamp(),
  constraint command_receipts_response_object check (pg_catalog.jsonb_typeof(response) = 'object')
);
...
create table agent_private.claims (
  ...
  constraint claims_kind_values check (kind in ('file', 'package', 'resource', 'manifest', 'device')),
  constraint claims_mode_values check (mode in ('exact', 'prefix')),
  constraint claims_prefix_file_only check (mode = 'exact' or kind = 'file'),
  constraint claims_value_length check (pg_catalog.length(value) between 1 and 512),
  constraint claims_release_shape check (
...
create table agent_private.events (
  ...
  created_at timestamptz not null default pg_catalog.clock_timestamp(),
  constraint events_type_values check (event_type in (
    'resume', 'started', 'claims_acquired', 'claims_released',
    'base_reconciled', 'blocked', 'review', 'done', 'cancelled',
    'claim_conflict', 'no_active_work'
  )),
  constraint events_payload_object check (pg_catalog.jsonb_typeof(payload) = 'object')
);
...
create function agent_private.reject_immutable_mutation()
returns trigger
language plpgsql
security invoker
...
create function agent_private.fail(p_code text, p_detail text default null)
returns void
language plpgsql
security invoker
set search_path = pg_catalog
...
create function agent_private.assert_object_keys(p_value jsonb, p_allowed text[])
returns void
language plpgsql
security invoker
set search_path = pg_catalog
...
create function agent_private.require_text(
  p_object jsonb,
  p_key text,
...
create function agent_private.optional_text(
  p_object jsonb,
  p_key text,
  p_max integer default 2000
)
...
create function agent_private.require_positive_bigint(p_object jsonb, p_key text)
returns bigint
language plpgsql
security invoker
set search_path = pg_catalog
...
create function agent_private.optional_positive_bigint(p_object jsonb, p_key text)
returns bigint
language plpgsql
security invoker
set search_path = pg_catalog
...
create function agent_private.assert_sha(p_value text, p_field text, p_nullable boolean default false)
returns text
language plpgsql
security invoker
set search_path = pg_catalog
...
create function agent_private.parse_session_name(p_session_name text)
...
create function agent_private.assert_agent_id(p_agent_id text, p_profile_key text, p_slot smallint)
returns text
language plpgsql
security invoker
set search_path = pg_catalog
...
begin
  ...
  from agent_private.profiles
  where profile_key = p_profile_key;

  if p_agent_id is null
     or p_agent_id !~ ('^' || v_prefix || '-' || p_slot::text || '-[0-9]{8}-[0-9]{4}-[a-z0-9]{4}$') then
...
create function agent_private.generate_agent_id(p_profile_key text, p_slot smallint)
returns text
language plpgsql
security invoker
...
begin
  ...
  for v_attempt in 1..16 loop
    ...
    if not exists (
      ...
      where project_id is not null and agent_id = v_agent_id
    ) then
      return v_agent_id;
    end if;
  end loop;
...
create function agent_private.assert_branch(p_branch text, p_issue_number bigint, p_nonce text)
returns text
language plpgsql
security invoker
set search_path = pg_catalog
...
begin
  if p_branch is null
     ...
     or p_branch ~ '@\{'
     or p_branch ~ '(^|/)[^/]*\.lock(/|$)'
     or pg_catalog.right(p_branch, 1) in ('/', '.') then
    perform agent_private.fail('invalid_branch');
  end if;
  v_nonce := pg_catalog.right(p_branch, 4);
  if p_nonce is not null and p_nonce <> v_nonce then
...
create function agent_private.assert_public_text(p_value text, p_field text)
returns text
language plpgsql
security invoker
set search_path = pg_catalog
as $$
begin
  if p_value ~* '(postgres(ql)?://|supabase\.co|authorization[[:space:]]*:[[:space:]]*bearer|service[_ -]?role|password[[:space:]]*[:=]|api[_ -]?key[[:space:]]*[:=]|secret[[:space:]]*[:=]|-----BEGIN [A-Z ]*PRIVATE KEY-----)' then
    perform agent_private.fail('sensitive_text_rejected', p_field);
  end if;
  return p_value;
end;
$$;

create function agent_private.normalize_claim_value(p_kind text, p_mode text, p_value text)
returns text
language plpgsql
security invoker
set search_path = pg_catalog
...
begin
  ...
  if p_value is null or pg_catalog.length(p_value) < 1 or pg_catalog.length(p_value) > 512 then
  ...
  if p_kind = 'file' then
    if p_value !~ '^[A-Za-z0-9._@+:-]+(/[A-Za-z0-9._@+:-]+)*$'
       ...
       or p_value ~ '//'
       or p_value ~ '\\'
       or pg_catalog.left(p_value, 1) = '/' then
      perform agent_private.fail('unsafe_path');
    end if;
  else
    if p_value !~ '^[A-Za-z0-9][A-Za-z0-9._:/@+-]*$'
       or p_value ~ '\.\.'
       or p_value ~ '//' then
      perform agent_private.fail('unsafe_claim_identifier');
    end if;
  end if;
  return p_value;
end;
...
create function agent_private.normalize_claims(p_claims jsonb, p_require_nonempty boolean)
returns jsonb
language plpgsql
security invoker
set search_path = pg_catalog
...
declare
  ...
  v_normalized jsonb;
begin
  if p_claims is null or pg_catalog.jsonb_typeof(p_claims) <> 'array' then
    perform agent_private.fail('claims_must_be_array');
  end if;
  if p_require_nonempty and pg_catalog.jsonb_array_length(p_claims) = 0 then
    perform agent_private.fail('claims_required');
  end if;
  if pg_catalog.jsonb_array_length(p_claims) > 256 then
    perform agent_private.fail('too_many_claims');
  end if;

  for v_claim in select value from pg_catalog.jsonb_array_elements(p_claims)
  loop
    perform agent_private.assert_object_keys(v_claim, array['kind', 'mode', 'value']);
    v_normalized := pg_catalog.jsonb_build_object(
      'kind', agent_private.require_text(v_claim, 'kind', 1, 16),
      'mode', agent_private.require_text(v_claim, 'mode', 1, 8),
      'value', agent_private.normalize_claim_value(
        v_claim ->> 'kind',
        v_claim ->> 'mode',
        agent_private.require_text(v_claim, 'value', 1, 512)
      )
    );
    if exists (select 1 from pg_catalog.jsonb_array_elements(v_result) existing where existing = v_normalized) then
      perform agent_private.fail('duplicate_claim');
    end if;
    v_result := v_result || pg_catalog.jsonb_build_array(v_normalized);
  end loop;
  return v_result;
end;
$$;

create function agent_private.find_claim_collision(
  p_project_id uuid,
  p_session_id uuid,
  p_kind text,
  p_mode text,
...
create function agent_private.session_snapshot(p_session_id uuid)
returns jsonb
language sql
stable
security invoker
...
create function agent_private.complete_request(
  p_request_id text,
  p_project_id uuid,
  p_session_id uuid,
  p_action text,
...
revoke all on all functions in schema agent_private
...
values
  ...
  ('agent-state-supabase', 'StreamScapeTV/agent-state-supabase', 'main');

insert into agent_private.project_slots(project_id, profile_key, slot)
select p.id, f.profile_key, series.slot::smallint
from agent_private.projects p
cross join agent_private.profiles f
cross join lateral pg_catalog.generate_series(1, 9) as series(slot);
