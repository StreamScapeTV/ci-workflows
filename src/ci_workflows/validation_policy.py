...
from .validation_helpers import (
    ...
    _validate_runner as _validate_semantic_runner,
)
...
def _contains_mutable_publication_tag(raw: str) -> bool:
    """Return true only when a publication command selects ``latest``."""

    latest = re.compile(
        r"(?:^|[^A-Za-z0-9_.-])latest(?:[^A-Za-z0-9_.-]|$)"
    )
    for line in raw.lower().splitlines():
        if any(word in line for word in _PUBLICATION_WORDS) and latest.search(
            line
        ):
            return True
    return False
...
def _validate_runner(
    ...
    if isinstance(runner, list):
        ...
        if (
            labels in config.allowed_runner_selectors
...
def _validate_workflow(
    ...
    data = document.data
    if not isinstance(data.get("name"), str) or not str(
        data.get("name")
    ).strip():
        _finding(
            findings,
            ...
            path,
            "workflow requires a human-readable name",
        )
    if "on" not in data:
        _finding(
            findings,
            ...
            "missing-trigger",
            path,
            "workflow must contain the literal on key",
        )
    if "permissions" not in data or data.get("permissions") in (
        None,
        "write-all",
    ):
        _finding(
    ...
    for permission, value in permissions.items():
        if value not in {"read", "write", "none"}:
            _finding(
                ...
                "invalid-permission",
                path,
                f"permission {permission!r} has invalid value {value!r}",
            )
    ...
    if _is_public_workflow(path) and events != {"workflow_call"}:
        _finding(
            findings,
            config,
            "public-workflow-trigger",
            path,
            "public reusable workflows must expose workflow_call only",
        )
    raw_lower = document.raw.lower()
    has_checkout = "actions/checkout@" in raw_lower
    if (
        has_checkout
        and "git rev-parse head" not in raw_lower
        and "./actions/exact-checkout" not in raw_lower
    ):
        _finding(
            findings,
            config,
            "missing-exact-head-assertion",
            path,
            "checkout workflows must assert the exact checked-out HEAD",
        )
    if events & _HIGH_RISK_EVENTS and has_checkout:
        _finding(
            ...
            path,
            f"high-risk events {sorted(events & _HIGH_RISK_EVENTS)} may not "
            "checkout or execute source",
        )
    ...
    if "pull_request" in events and any(
        word in raw_lower for word in _PUBLICATION_WORDS
    ):
        _finding(
            findings,
            config,
            "publication-from-pr",
            path,
            "pull-request workflows may not publish images or charts",
        )
    if any(word in raw_lower for word in _PUBLICATION_WORDS):
        if _contains_mutable_publication_tag(document.raw):
            _finding(
                ...
                "mutable-release-tag",
                path,
                "publication may not use latest",
            )
        if not any(word in raw_lower for word in _READBACK_WORDS):
            _finding(
                findings,
                config,
                "missing-publication-readback",
                path,
                "publication requires independent remote read-back",
            )
    if any(
        word in raw_lower for word in _CREDENTIAL_WORDS + _TEMP_STATE_WORDS
    ):
        cleanup_steps = [
            step
        ...
        if not cleanup_steps:
            _finding(
                ...
                path,
                "credential or temporary state requires an if: always() "
                "cleanup step",
            )
    ...
    for job_id, job in _iter_jobs(document):
        if not _JOB_ID_RE.fullmatch(job_id):
            _finding(
                ...
                "opaque-job-id",
                path,
                f"job id {job_id!r} is not readable snake_case",
            )
        if not isinstance(job.get("name"), str) or not str(
            job.get("name")
        ).strip():
            _finding(
                findings,
                ...
                "opaque-job-name",
                path,
                f"job {job_id!r} requires a human-readable name",
            )
        ...
        if is_reusable_call:
            if "runs-on" in job or "steps" in job:
                _finding(
                    ...
                    path,
                    f"job {job_id!r} mixes uses with runs-on/steps",
                )
        else:
            if "runs-on" not in job:
                _finding(
                    ...
                    "missing-runner",
                    path,
                    f"job {job_id!r} requires runs-on",
                )
            else:
                _validate_runner(
                    job.get("runs-on"),
        ...
        if isinstance(strategy, Mapping) and "matrix" in strategy:
            ...
            if isinstance(matrix, str) and "${{" in matrix:
                _finding(
                    ...
                    "opaque-matrix",
                    path,
                    f"job {job_id!r} uses an opaque dynamic matrix",
                )
            else:
                ...
                if size is None:
                    _finding(
                        ...
                        "unbounded-matrix",
                        path,
                        f"job {job_id!r} matrix cannot be bounded statically",
                    )
                else:
                    ...
                    if size > contract_limit:
                        _finding(
                            ...
                            "unbounded-matrix",
                            path,
                            f"job {job_id!r} matrix size {size} exceeds "
                            f"{contract_limit}",
                        )
        for step_index, step in _iter_steps(job):
            if not isinstance(step.get("name"), str) or not str(
                step.get("name")
            ).strip():
                _finding(
                    findings,
            ...
            if isinstance(run, str):
                ...
                if line_count > config.max_inline_run_lines:
                    _finding(
                        ...
                        path,
                        f"job {job_id!r} step {step_index} has {line_count} "
                        "non-empty run lines; move logic to a named "
                        "function/action",
                    )
                ...
                if (
                    ...
                    _finding(
                        ...
                        "complex-yaml-logic",
                        path,
                        "non-trivial loops belong in named tested functions",
                    )
                if re.search(
                    r"(^|\n)\s*(?:function\s+)?"
                    ...
                    _finding(
                        ...
                        "complex-yaml-logic",
                        path,
                        "shell function definitions belong in named tested "
                        "functions",
                    )
    ...
    if _is_public_workflow(path) and concurrency is not None:
        policy = (
            str(record.get("concurrency_policy", "caller-owned"))
            if record
            else "caller-owned"
        )
        if policy == "caller-owned":
            _finding(
                ...
                path,
                "public workflow must not own caller cancellation/concurrency",
            )
    if _is_public_workflow(path):
        _validate_public_compatibility(
            document,
            public_contract,
            config,
            findings,
...
def _validate_action(
    ...
    if not isinstance(data.get("name"), str) or not str(
        ...
        _finding(
            ...
            "opaque-action-name",
            path,
            "composite action requires a name",
        )
    runs = data.get("runs", {})
    if not isinstance(runs, Mapping) or runs.get("using") != "composite":
        _finding(
            ...
            path,
            "approved internal actions must be composite",
        )
        return
    ...
    if not isinstance(steps, list):
        _finding(
            ...
            "invalid-internal-action",
            path,
            "composite action steps must be a list",
        )
        return
    ...
    for index, step in enumerate(steps, start=1):
        ...
        if not isinstance(step.get("name"), str) or not str(
            ...
            _finding(
                ...
                "opaque-step-name",
                path,
                f"composite step {index} requires a name",
            )
        run = step.get("run")
        if isinstance(run, str):
            line_count = _run_line_count(run)
            if line_count > config.max_inline_run_lines:
                _finding(
                    ...
                    "oversized-inline-run",
                    path,
                    f"composite step {index} has {line_count} run lines",
                )
            normalized = _normalize_run(run)
        ...
        if len(unique_paths) > 1:
            for path in unique_paths:
                _finding(
                    ...
                    path,
                    f"implementation block is duplicated across "
                    f"{unique_paths}",
                )
