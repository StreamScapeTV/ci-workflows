...
def _validate_workflow(
    ...
    if any(word in raw_lower for word in _PUBLICATION_WORDS):
        if re.search(r"(?:^|[^A-Za-z0-9])latest(?:[^A-Za-z0-9]|$)", raw_lower):
            _finding(
                findings,
                config,
                "mutable-release-tag",
    ...
    if any(word in raw_lower for word in _CREDENTIAL_WORDS + _TEMP_STATE_WORDS):
        ...
        if not cleanup_steps:
            _finding(
                ...
                path,
                "credential or temporary state requires an if: always() cleanup step",
            )

    run_occurrences: collections.Counter[str] = collections.Counter()
    for job_id, job in _iter_jobs(document):
        ...
        if not isinstance(timeout, int) or timeout <= 0:
            ...
            _finding(
                ...
                "excessive-timeout",
                path,
                f"job {job_id!r} timeout {timeout} exceeds {config.max_timeout_minutes}",
            )
            ...
            if isinstance(matrix, str) and "${{" in matrix:
                ...
                if size is None:
                    ...
                    if record is not None:
                        contract_limit = min(
                            contract_limit,
                            int(record.get("matrix_max_jobs", contract_limit)),
                        )
                    if size > contract_limit:
                        _finding(
                            ...
                            "unbounded-matrix",
                            path,
                            f"job {job_id!r} matrix size {size} exceeds {contract_limit}",
                        )
            ...
            if isinstance(uses, str) and uses.startswith("actions/upload-artifact@"):
                ...
                if not isinstance(retention, int) or retention > 7:
                    _finding(
                        ...
                        path,
                        "artifact retention must be explicit and no more than seven days",
                    )
            run = step.get("run")
            if isinstance(run, str):
                ...
                if re.search(
                    ...
                    _finding(
                        ...
                        "complex-yaml-logic",
                        path,
                        "shell function definitions belong in named tested functions",
                    )
    for count in run_occurrences.values():
        if count > 1:
            _finding(
                findings,
                config,
                ...
                path,
                f"same implementation block is repeated {count} times in one workflow",
            )
    ...
    for index, step in enumerate(steps, start=1):
        ...
        if isinstance(run, str):
            ...
            if line_count >= 8 and normalized:
                ...
                duplicated_runs.setdefault(normalized, []).append(path)
    for count in local_runs.values():
        if count > 1:
            _finding(
                findings,
                config,
                "duplicated-inline-run",
                path,
                f"same implementation block is repeated {count} times in one composite action",
            )
    for uses, comment, line_number in _uses_entries(document):
        _validate_action_reference(
            uses,
        ...
        if len(unique_paths) > 1:
            for path in unique_paths:
                _finding(
                    ...
                    path,
                    f"implementation block is duplicated across {unique_paths}",
                )
