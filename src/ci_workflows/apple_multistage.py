"""Bounded caller-owned Apple protected-full planning and execution.

This module adds the single-executor Apple path without changing the legacy
profile/consumer-contract API.  It deliberately reuses the reviewed Apple
execution primitives for toolchain validation, signing lockdown, simulator
ownership, stale-simulator reconciliation, command execution, and no-follow
cleanup.
"""
...
_STAGE_ID = re.compile(r"^[a-z][a-z0-9-]{1,31}$")
...
_MAX_STAGES = 8
_MAX_LIST_ITEMS = 24
_PLATFORMS = {"ios", "tvos", "macos", "source"}
...
def _strings(
    ...
    *,
    ...
    if not isinstance(value, list) or len(value) > maximum_items or (not allow_empty and not value):
        _fail(code)
    result = tuple(_plain(item, code, maximum=2048) for item in value)
    if len(result) != len(set(result)):
        _fail(code)
    return result


def _relative(value: object, code: str, *, allow_dot: bool = False, allow_empty: bool = False) -> str:
    if allow_empty and value == "":
        return ""
    try:
        return safe_relative(_plain(value, code, maximum=1024), code, allow_dot=allow_dot)
...
def _arguments(value: object) -> tuple[str, ...]:
    arguments = _strings(value, "validation_plan_invalid")
    ...
    if any(argument.startswith("-") and "=" in argument for argument in arguments):
        # Keep public settings bounded to ordinary argv tokens rather than
        # caller-defined option/value channels.
        _fail("validation_plan_invalid")
    return arguments


def _cleanup_paths(value: object) -> tuple[str, ...]:
    return tuple(
        _relative(item, "cleanup_failed")
        for item in _strings(value, "cleanup_failed", maximum_items=16)
    )


def _expected_outputs(value: object) -> tuple[str, ...]:
    return tuple(
        _relative(item, "output_invalid")
        for item in _strings(value, "output_invalid", maximum_items=16)
    )
...
def _profile_for(platform: str, operation: str) -> AppleProfile:
    if operation == "script":
        if platform != "source":
            _fail("validation_plan_invalid")
...
class ProtectedApplePlan:
    ...
    def simulator_plans(self) -> tuple[AppleValidationPlan, ...]:
        return tuple(stage.plan for stage in self.stages if stage.plan.simulator is not None)
...
def _stage_plan(
    ...
    *,
    ...
    if _STAGE_ID.fullmatch(identifier) is None:
        _fail("validation_plan_invalid")
    ...
    if platform not in _PLATFORMS or operation not in {*_XCODE_OPERATIONS, "script"}:
        _fail("validation_plan_invalid")
    working_directory = _relative(raw["working_directory"], "working_directory_rejected", allow_dot=True)
    cleanup_paths = _cleanup_paths(raw["cleanup_paths"])
    expected_outputs = _expected_outputs(raw["expected_outputs"])
    profile = _profile_for(platform, operation)

    if operation == "script":
        if raw["container"] is not None or raw["test_plan"] not in {None, ""}:
            _fail("validation_plan_invalid")
        if raw["scheme"] not in {None, ""} or raw["configuration"] not in {None, ""}:
            _fail("validation_plan_invalid")
    ...
    if platform == "source" or raw["script"] is not None:
        _fail("validation_plan_invalid")
    ...
    if _SCHEME.fullmatch(scheme) is None or _CONFIGURATION.fullmatch(configuration) is None:
        _fail("validation_plan_invalid")
    test_plan_raw = raw["test_plan"]
    test_plan = None
    if test_plan_raw not in {None, ""}:
        test_plan = _relative(test_plan_raw, "test_plan_rejected")
        if not test_plan.endswith(".xctestplan"):
            _fail("test_plan_rejected")
    resolution = _plain(raw["package_resolution_mode"], "package_resolution_rejected", maximum=32)
    if resolution not in {"locked", "disabled", "resolve-only"}:
        _fail("package_resolution_rejected")
    resolved_files = tuple(
        _relative(item, "package_resolution_rejected")
        for item in _strings(raw["resolved_files"], "package_resolution_rejected", maximum_items=8)
    )
    if resolution == "locked" and not resolved_files:
        _fail("package_resolution_rejected")
    arguments = list(_arguments(raw["xcodebuild_arguments"]))
    selectors = _strings(raw["test_selectors"], "test_selector_rejected", maximum_items=16)
...
def build_protected_full_plan(
    ...
    *,
    ...
    if len(stages) != len(raw_stages) or len({stage.identifier for stage in stages}) != len(stages):
        _fail("validation_plan_invalid")
    ...
    dep_sha = _plain(private_dependency_sha, "private_dependency_invalid", allow_empty=True, maximum=40)
    dep_id = _plain(private_dependency_id, "private_dependency_invalid", allow_empty=True, maximum=64)
    dep_subdirectory = _relative(
    ...
    if any(dependency_values):
        if not all(dependency_values) or _REPOSITORY.fullmatch(dep_repository) is None or _FULL_SHA.fullmatch(dep_sha) is None:
            _fail("private_dependency_invalid")
        if _STAGE_ID.fullmatch(dep_id) is None:
            _fail("private_dependency_invalid")
...
def verify_private_dependency(
    ...
    *,
    ...
    if (
        ...
        or environment.get("INPUT_PRIVATE_DEPENDENCY_CHECKOUT_REPOSITORY") != plan.private_dependency_repository
        or environment.get("INPUT_PRIVATE_DEPENDENCY_CHECKOUT_ID") != plan.private_dependency_id
        or environment.get("INPUT_PRIVATE_DEPENDENCY_EXPECTED_SUBPATH") != plan.private_dependency_subdirectory
    ):
...
def _protected_before(plan: ProtectedApplePlan, source_root: Path) -> tuple[dict[str, str], ...]:
    return tuple(apple_execution.protected_hashes(source_root, stage.plan) for stage in plan.stages)
...
def execute_protected_full(
    ...
    *,
    ...
    before = _protected_before(plan, source_root)
    env, directories = apple_execution.isolated_environment(state_root, base_environment)
    ...
    first_xcode = next(stage for stage in plan.stages if stage.operation in _XCODE_OPERATIONS)
    xcode_version, xcode_build, swift_version, sdk_versions, _ = apple_execution.verify_toolchain(
        first_xcode.plan,
        source_root,
        state_root,
        command_runner,
    ...
    needs_ownership = any(stage.plan.simulator is not None for stage in plan.stages)
    ownership_context = (
    ...
    return {
        ...
        "validation_profile": "protected-full",
        "task_profile": "protected-full",
        "test_summary": json.dumps(summaries, sort_keys=True, separators=(",", ":")),
        "runner_profile": AppleRunnerCapability.APPLE.value,
        "source_trust": plan.source_trust,
        "xcode_version": xcode_version,
        "xcode_build": xcode_build,
        "swift_version": swift_version,
        "sdk_versions_json": json.dumps(dict(sorted(sdk_versions.items())), sort_keys=True, separators=(",", ":")),
        "simulator_identity": "",
        "output_verified": str(any(bool(row["output_verified"]) for row in summaries)).lower(),
...
def _cleanup_targets(plan: ProtectedApplePlan, source_root: Path, state_root: Path) -> tuple[Path, ...]:
    targets: list[Path] = [apple_execution._lexical_target(state_root, "apple-validation")]
    for stage in plan.stages:
        targets.extend(
...
def cleanup_protected_full(
    ...
    *,
    ...
    simulator_plans = plan.simulator_plans
    ownership_context = (
        apple_execution._simulator_ownership(env, state_root)
        if simulator_plans
        else nullcontext(None)
    ...
    with ownership_context as ownership:
        if simulator_plans:
            assert ownership is not None
            for stage_plan in simulator_plans:
                apple_execution._cleanup_simulator_locked(
                    source_root,
                    state_root,
                    stage_plan,
...
def assert_zero_protected_full_residue(
    ...
    *,
    ...
    remaining = [path for path in _cleanup_targets(plan, source_root, state_root) if apple_execution._lstat(path) is not None]
    if remaining:
        _fail("cleanup_failed")
    simulator_plans = plan.simulator_plans
    if not simulator_plans:
        return
    command_runner = runner or apple_execution.SubprocessCommandRunner()
    env = dict(environment or {})
    with apple_execution._simulator_ownership(env, state_root) as ownership:
        ...
        for stage_plan in simulator_plans:
            ...
            if apple_execution._exact_owned_candidates(stage_plan, payload, require_available=False):
                _fail("cleanup_failed")
