...
def _state_root(context: CIWContext) -> Path:
    try:
        ...
        raise android_validation.AndroidValidationError("invalid_input") from error
    temporary = root / "tmp"
...
def execute_android_validate(args: argparse.Namespace, context: CIWContext) -> CIWResult:
    ...
    try:
        ...
        if args.phase == "cleanup":
            ...
            return CIWResult("android", "validate", outputs={"cleanup_result": "success", "failure_code": ""})
        if args.phase == "residue":
            assert state_root is not None
            for relative in ("android-validation", "android-source"):
                path = state_root / relative
                if path.exists() or path.is_symlink():
                    raise android_validation.AndroidValidationError("cleanup_failed")
            return CIWResult("android", "validate", outputs={"cleanup_result": "success", "failure_code": ""})

        request = android_validation.request_from_environment(context.environment, contract)
        if args.phase == "plan":
            ...
            if not isinstance(plan, android_validation.AndroidValidationPlan):
                raise android_validation.AndroidValidationError("invalid_input")
        ...
        source_root = android_validation.bounded_path(workspace, android_validation.safe_relative(args.source_root))
        assert state_root is not None
        ...
        if not isinstance(result, android_validation.AndroidValidationResult):
            raise android_validation.AndroidValidationError("invalid_input")
        return CIWResult("android", "validate", outputs=result.output_values())
