...
def _state_root(context: CIWContext) -> Path:
    ...
    root = resolve_state_root(runner_temp=runner_temp, state_id=state_id, declared_root=declared_root, contract_root=context.root)
    temporary = root / "tmp"
...
def _failure_outputs(context: CIWContext, code: str) -> None:
    ...
    if not path:
        return
    summary = json.dumps({"status":"failed", "validation_profile":context.environment.get("INPUT_VALIDATION_PROFILE", ""), "script_contract":"consumer-owned-executable"}, sort_keys=True, separators=(",", ":"))
    write_command_file(Path(path), {"result":"failure", "test_summary":summary, "source_sha":context.environment.get("INPUT_ADMITTED_SHA", ""), "resolved_python_version":"", "validation_profile":context.environment.get("INPUT_VALIDATION_PROFILE", ""), "cleanup_result":"not-run", "failure_code":code, "artifact_exception_used":"false", "evidence_id":"", "runner_profile":"", "runs_on_json":"", "workspace_profile":"", "timeout_minutes":"", "source_trust":""})


def execute_python_validate(args: argparse.Namespace, context: CIWContext) -> CIWResult:
    try:
        request = python_validation.request_from_environment(context.environment)
        if args.phase == "plan":
            plan = python_validation.validate(contract_root=context.root, source_root=None, state_root=None, request=request, phase="plan", environment=context.environment)
            if not isinstance(plan, python_validation.PythonValidationPlan):
                raise python_validation.PythonValidationError("invalid_input")
            organization = runners.resolve_runner_profile(runners.load_runner_contract(context.root), workflow_api="validation.python", source_trust=request.source_trust, requested_profile=plan.runner_profile)
            try:
                backend = resolve_execution_backend(execution_backend=context.environment.get("INPUT_EXECUTION_BACKEND", "organization"), execution_profile=organization.execution_profile, organization_runs_on=organization.runs_on)
            ...
            outputs.update({"runner_profile":plan.runner_profile, "runs_on_json":backend.as_dict()["runs_on_json"], "workspace_profile":plan.workspace_profile, "timeout_minutes":str(plan.timeout_minutes), "source_trust":request.source_trust, "evidence_id":""})
        ...
        result = python_validation.validate(contract_root=context.root, source_root=source_root, state_root=_state_root(context), request=request, phase="execute", environment=context.environment)
