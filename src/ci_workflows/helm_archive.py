...
def canonicalize_chart_archive_bytes(
    ...
    *,
    ...
    """Canonicalize one chart archive and every packaged subchart recursively."""
    ...
    try:
        with tarfile.open(fileobj=io.BytesIO(content), mode="r:gz") as archive:
            ...
            require(
                bool(raw_members) and len(raw_members) <= _MAX_MEMBERS,
                "archive_invalid",
            )
            ...
            for member in raw_members:
                ...
                expanded += len(data)
                require(expanded <= _MAX_EXPANDED_BYTES, "archive_invalid")
...
def canonicalize_chart_archive(
    source: Path,
    destination: Path,
    chart_name: str,
) -> str:
    ...
    try:
        ...
        canonical = canonicalize_chart_archive_bytes(
            content,
            expected_root=chart_name,
        )
