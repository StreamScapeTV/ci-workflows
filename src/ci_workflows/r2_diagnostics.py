"""Upload one bounded private CI diagnostic to Cloudflare R2 and verify read-back."""
...
def _validated_object_key(request_id: str, run_id: int, attempt: int) -> str:
    ...
    _require(isinstance(run_id, int) and run_id > 0, "invalid_run_id")
    _require(
...
def upload_private_diagnostic(
    *,
    ...
    get_headers = _authorization_headers(
        method="GET",
        url=url,
    ...
    try:
        with opener(get_request, timeout=_TIMEOUT_SECONDS) as response:
            readback = response.read(MAX_COMPRESSED_BYTES + 1)
    except urllib.error.HTTPError as error:
        raise R2DiagnosticError(f"r2_readback_http_{error.code}") from None
    except (OSError, urllib.error.URLError, ValueError):
        raise R2DiagnosticError("r2_readback_unavailable") from None

    _require(len(readback) <= MAX_COMPRESSED_BYTES, "r2_readback_too_large")
    _require(len(readback) == len(compressed), "r2_readback_size_mismatch")
    _require(
        hmac.compare_digest(_sha256(readback), digest),
