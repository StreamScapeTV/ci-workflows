...
The backend is runner-infrastructure owned. Callers never select a backend or
filesystem path: trusted runner/device infrastructure provides
``CIW_DEVICE_LOCK_ROOT`` and every execution context that can reach the same
physical device must share that root. Persisted state contains only hashed
identities, exact non-secret request facts, lease epochs, and opaque fences.
"""
...
class DeviceLockError(ValueError):
    """Fail closed with one stable, non-sensitive public code."""
...
def _opaque_text(value: object, *, minimum: int = 8, maximum: int = 4096) -> str:
    ...
    text = str(value)
    _require(minimum <= len(text) <= maximum, "invalid_input")
    _require(not any(character in text for character in ("\x00", "\r", "\n")), "invalid_input")
    return text
...
def load_device_lock_contract(root: Path) -> DeviceLockContract:
    ...
    _require(
        ...
        and set(lease) == {"minimum_seconds", "maximum_seconds"},
        "contract_invalid",
    )
    ...
    _require(isinstance(families, list) and tuple(families) == ("android", "ios", "tvos"), "contract_invalid")
...
class DeviceLockRequest:
    ...
    def resource_key_hash(self) -> str:
        """Hash one exact physical device, independent of requested capability."""

        return _sha256_text(f"{self.device_family}\0{self.device_identity_hash}")
...
class DeviceLockReceipt:
    ...
    @classmethod
    def from_payload(cls, contract: DeviceLockContract, payload: Mapping[str, object]) -> "DeviceLockReceipt":
        ...
        _require(
            contract.minimum_lease_seconds <= duration <= contract.maximum_lease_seconds,
            "receipt_invalid",
        )
        return cls(
            receipt_version=str(payload["receipt_version"]),
...
def _release_evidence(receipt: DeviceLockReceipt) -> DeviceLockReleaseEvidence:
...
def _validate_release_marker(contract: DeviceLockContract, payload: Mapping[str, object]) -> DeviceLockReleaseEvidence:
...
def _safe_control_directory(path: Path, *, create: bool) -> None:
    ...
    _require(info.st_uid == os.geteuid() and _mode(info) == 0o700, "backend_unavailable")


def resolve_backend_root(contract: DeviceLockContract, environment: Mapping[str, str] | None = None) -> Path:
...
def _write_json_file(path: Path, payload: Mapping[str, object]) -> None:
    ...
    descriptor = -1
    try:
        descriptor = os.open(temporary, _open_flags(os.O_CREAT | os.O_EXCL | os.O_WRONLY), 0o600)
        offset = 0
        while offset < len(encoded):
            written = os.write(descriptor, encoded[offset:])
            _require(written > 0, "backend_unavailable")
            offset += written
        os.fsync(descriptor)
        os.close(descriptor)
        descriptor = -1
...
def _transaction(root: Path, contract: DeviceLockContract) -> Iterator[None]:
    ...
    locked = False
    try:
        descriptor = os.open(path, _open_flags(os.O_CREAT | os.O_RDWR), 0o600)
        info = os.fstat(descriptor)
        _require(stat.S_ISREG(info.st_mode), "backend_corrupt")
        _require(info.st_uid == os.geteuid() and _mode(info) == 0o600, "backend_corrupt")
...
def _matches_request(receipt: DeviceLockReceipt, request: DeviceLockRequest) -> bool:
    return (
        ...
        and receipt.expires_at_epoch - receipt.acquired_at_epoch == request.lease_seconds
    )
...
class PosixDeviceLockBackend:
    ...
    def _current(self, resource_key_hash: str) -> DeviceLockReceipt | None:
        ...
        return None if payload is None else DeviceLockReceipt.from_payload(self.contract, payload)
    ...
    def _last_release(self, resource_key_hash: str) -> DeviceLockReleaseEvidence | None:
        ...
        return None if payload is None else _validate_release_marker(self.contract, payload)
    ...
    def verify(
        ...
        *,
        ...
        _require(
            isinstance(minimum_remaining_seconds, int) and minimum_remaining_seconds >= 0,
            "lease_rejected",
        )
    ...
    def release(self, encoded_receipt: str, request: DeviceLockRequest) -> DeviceLockReleaseEvidence:
        receipt = DeviceLockReceipt.decode(self.contract, encoded_receipt)
        _require(_matches_request(receipt, request), "receipt_mismatch")
    ...
    def assert_released(self, encoded_receipt: str, request: DeviceLockRequest) -> DeviceLockReleaseEvidence:
        receipt = DeviceLockReceipt.decode(self.contract, encoded_receipt)
        _require(_matches_request(receipt, request), "receipt_mismatch")
        expected = _release_evidence(receipt)
        with _transaction(self.root, self.contract):
            _require(self._current(request.resource_key_hash) is None, "lock_residue")
            previous = self._last_release(request.resource_key_hash)
...
@contextlib.contextmanager
def locked_device_session(backend: PosixDeviceLockBackend, request: DeviceLockRequest) -> Iterator[DeviceLockReceipt]:
