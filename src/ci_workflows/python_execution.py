...
def _classify_script_failure(output: str) -> str:
    "Return only a fixed safe category for captured product-script output."
...
def host_python_version(executable: str, cwd: Path, environment: Mapping[str, str]) -> str:
    completed = run_command([executable, "-c", "import platform,sys;print(f'{sys.implementation.name}|{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}|{platform.system()}|{platform.machine()}')"], cwd=cwd, environment=environment, timeout_seconds=30, code="toolchain_mismatch", failure_stage="runtime-container-setup")
...
def command_environment(plan: PythonValidationPlan, base: Mapping[str, str]) -> dict[str, str]:
    ...
    result.update({"LC_ALL": "C.UTF-8", "LANG": "C.UTF-8", "PYTHONDONTWRITEBYTECODE": "1", "PIP_DISABLE_PIP_VERSION_CHECK": "1", "PIP_NO_CACHE_DIR": "1"})
    result.update(plan.environment)
    return result


def execute_host_plan(source_root: Path, state_root: Path, plan: PythonValidationPlan, environment: Mapping[str, str]) -> int:
    ...
    require(host_python_version(sys.executable, cwd, execution_environment) == plan.python_version, "python_version_drift")
    executable = sys.executable
    if plan.dependency_file is not None:
        venv = isolated / "venv"
        run_command([sys.executable, "-m", "venv", str(venv)], cwd=cwd, environment=execution_environment, timeout_seconds=120, code="dependency_restore_failed", failure_stage="dependency-restoration")
        executable = str(venv / "bin" / "python")
        execution_environment["VIRTUAL_ENV"] = str(venv)
        execution_environment["PATH"] = f"{venv / 'bin'}:{execution_environment.get('PATH', '')}"
        run_command([executable, "-m", "pip", "install", "--no-input", "--no-cache-dir", "-r", str(bounded_path(work, plan.dependency_file))], cwd=cwd, environment=execution_environment, timeout_seconds=max(60, plan.timeout_minutes * 30), code="dependency_restore_failed", failure_stage="dependency-restoration")
    for command in plan.commands:
        argv = tuple(executable if item in {"python", "python3"} else item for item in command.argv)
        run_command(argv, cwd=cwd, environment=execution_environment, timeout_seconds=plan.timeout_minutes * 60, failure_stage=_failure_stage_for_command(command.stage))
...
def container_script(plan: PythonValidationPlan) -> str:
    ...
    lines = ["set -eu", f"cp -a /src/. /work/source || exit {_PODMAN_RUNTIME_SETUP_EXIT}", f"cd {shlex.quote(target)} || exit {_PODMAN_RUNTIME_SETUP_EXIT}", f"python -m venv /work/venv || exit {_PODMAN_RUNTIME_SETUP_EXIT}"]
    if plan.dependency_file is not None:
    ...
    for command in plan.commands:
        exit_code = _PODMAN_SCRIPT_INVOCATION_EXIT if command.stage == "release-contract" else _PODMAN_PRODUCT_COMMAND_EXIT
...
def _database_url_scheme(plan: PythonValidationPlan, contract: Mapping[str, Any]) -> str:
    try:
        consumer = contract["consumers"][plan.repository]["profiles"][plan.command_profile]
    except (KeyError, TypeError) as error:
        raise PythonValidationError("invalid_input") from error
    scheme = consumer.get("database_url_scheme", "postgresql")
    require(isinstance(scheme, str) and scheme in _DATABASE_URL_SCHEMES, "invalid_input")
    return scheme
...
def _start_postgres(podman: Sequence[str], *, source_root: Path, state_root: Path, plan: PythonValidationPlan, contract: Mapping[str, Any], environment: Mapping[str, str], container: str, network: str, volume: str) -> str:
    ...
    return f"{_database_url_scheme(plan, contract)}://{postgres['username']}:{password}@{postgres['network_alias']}:5432/{postgres['database']}"
...
def _raise_podman_execution_failure(returncode: int, output: str = "") -> None:
    if returncode == _PODMAN_DEPENDENCY_RESTORE_EXIT:
        ...
        code, stage, diagnostic = "command_failed", "product-command", "command_failed"
    else:
...
def execute_podman_plan(source_root: Path, state_root: Path, plan: PythonValidationPlan, contract: Mapping[str, Any], environment: Mapping[str, str]) -> int:
    ...
    try:
        validation_environment = {**plan.environment, "PYTHONDONTWRITEBYTECODE": "1", "PIP_DISABLE_PIP_VERSION_CHECK": "1", "PIP_NO_CACHE_DIR": "1"}
...
def result_from_plan(plan: PythonValidationPlan, stage_count: int) -> PythonValidationResult:
    material = {"source_sha": plan.admitted_sha, "validation_profile": plan.validation_profile, "command_profile": plan.command_profile, "python_version": plan.python_version, "stage_count": stage_count, "cleanup": "registered"}
    return PythonValidationResult(source_sha=plan.admitted_sha, resolved_python_version=plan.python_version, validation_profile=plan.validation_profile, command_profile=plan.command_profile, stage_count=stage_count, cleanup_result="registered", evidence_id=stable_identifier("python", material, length=28))